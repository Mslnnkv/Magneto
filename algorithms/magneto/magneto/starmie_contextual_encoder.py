...
class StarmieContextualEncoder:
    def __init__(
        ...
        supported_modes = {"table_context_window_starmie_marker"}
    ...
    def encode(self, df, target_col):
        ...
        text = self._build_window_serialization(df, window_columns)
    ...
    def _build_window_serialization(self, df, window_columns):
        parts = ["Window Table Representation."]

        for col in window_columns:
            parts.append(self._build_column_block(df, col))

        return "\n".join(parts)

    def _build_column_block(self, df, col):
