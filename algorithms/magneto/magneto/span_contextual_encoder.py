...
class SpanContextualEncoder:
    def __init__(
        ...
        self.target_start_marker = target_start_marker
        self.target_end_marker = target_end_marker

        supported_modes = {"table_context_window_span"}
        if self.encoding_mode not in supported_modes:
            raise ValueError(
                f"Unsupported span encoding mode: {self.encoding_mode}. "
    ...
    def encode(self, df, target_col):
        ...
        start_marker = self.target_start_marker
        end_marker = self.target_end_marker

        start_idx = text.find(start_marker)
        end_idx = text.find(end_marker)

        if start_idx == -1 or end_idx == -1:
            raise ValueError("Target markers not found in serialized text.")
        ...
        return {
            ...
            "target_start_marker": start_marker,
            "target_end_marker": end_marker,
        }
    ...
    def _build_window_serialization(self, df, target_col):
        ...
        target_header = (
            f"{self.target_start_marker} {target_col} {self.target_end_marker}"
        )

        parts = [
            "Window Context.",
            f"Left Columns: {left_text}.",
            f"Target Column Header: {target_header}.",
        ...
        if self.include_target_values:
            ...
            parts.append(f"Target Sample Values: {values_text}.")
