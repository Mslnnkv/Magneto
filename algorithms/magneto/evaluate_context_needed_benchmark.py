...
def main():
    source_df = pd.read_csv(BENCHMARK_DIR / "context_needed_source.csv")
    target_df = pd.read_csv(BENCHMARK_DIR / "context_needed_target.csv")
    gt_map = load_ground_truth(BENCHMARK_DIR / "context_needed_ground_truth.csv")
    ...
    for mode in ["header_values_verbose", "table_context_window_span"]:
        ...
        for source_col, gt_target in gt_map.items():
            ...
            print(f"{source_col:8s} | gt={gt_target:8s} | preds={preds}")
            shown += 1
            if shown >= 12:
                break
    ...
    print("\nFinal results:")
    print(results_df)
    results_df.to_csv(BENCHMARK_DIR / "context_needed_results.csv", index=False)
